...
function Member(nombre, id) {
    this.id = id;
    this.name = name;
}

function Loan(id, idMember, idBook, days) {
    this.id = id;
...
module.exports = {
    ...
    getBookId: (req, res) => {
        ...
        if (book != 0) {
            res.status(200).json({ "BookId": parseInt(req.params.BookId), "Availables": book.availables() });
        }
        else {
    ...
    deleteBook: (req, res) => {
        let result = f.deleteBook(req.params.id);
        if (result) {
    ...
    putBook: (req,res)=>{
        let result = f.updateBook(req.params.BookId, req.body.quantity);
        if (result == 1) {
            res.status(200).json({data:f.findID(req.params.idLibro, libros)});
        }
        else if (result == -1) {
            res.status(400).json(
                {
                    error:{
                        code:400,
                        message:"the amount of books is lower than the amount of books borrowed"
                    }
                })
        }
        else {
    ...
    getLoansMember: (req, res) => {
        console.log(`GET Loans by Member`);
        if (f.findID(req.params.idMember, members)) {
            let prest = f.getloans(req.params.idMember);
            if (prest.length == 0) {
                res.status(200).json(`There are no loans made by member id: ${req.params.id}`);
            }
            else {
